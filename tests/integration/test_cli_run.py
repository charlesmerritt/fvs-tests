...
def test_cli_rejects_baseline_with_no_comparison_tables(tmp_path: Path) -> None:
    repository = tmp_path / "repository"
    example = repository / "examples" / "minimal"
    expected_directory = example / "expected"
    expected_directory.mkdir(parents=True)
    ...
    assert "comparison policy must declare at least one table" in completed.stderr
    assert "comparison: equivalent" not in completed.stdout
